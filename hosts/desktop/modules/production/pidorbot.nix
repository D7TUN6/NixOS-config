{
  ...
  cfg = config.services.pidorbot;
  botDir = "/home/d7tun6/files/mounts/TS480SSD/services/pidorbot";
...
in {
  options.services.pidorbot = {
    enable = lib.mkEnableOption "pidorbot - random user selection telegram bot";

    dbPath = lib.mkOption {
      type = lib.types.str;
      default = "${botDir}/pidorbot.db";
      description = "path to sqlite database (DB_PATH)";
    };
  };

  config = {
    services.pidorbot.enable = lib.mkDefault true;

    environment.systemPackages = lib.mkIf cfg.enable [botEnv];

    systemd.services.pidorbot = lib.mkIf cfg.enable {
      description = "pidorbot - random user selection telegram bot";
      after = [
      ...
      serviceConfig = {
        ...
        EnvironmentFile = [config.sops.secrets.pidorbot-token.path];
